...
- [Non-standard commands](#non-standard-commands)
  ...
  - [`eval`](#eval)
  - [`compiler`](#compiler)
  - [`export_net [filename]`](#export_net-filename)
  - [`flip`](#flip)
  - [`help`](#help)
  - [`license`](#license)
...
List of options:
  ...
  * #### `nodestime`
    ...
    Tells the engine to use nodes searched instead of wall time to account for elapsed time. Useful for engine testing. When this option is set, the engine's allocated thinking time is tracked as a nodes budget rather than in milliseconds. The precise mechanism works as follows. During a game, for every move, the engine gets a time control value `time`, and possibly a time increment value `inc`, both in milliseconds. The available nodes budget is initialized once, at the start of the game (`availableNodes = time * nodestime`), and the value of `time` is ignored for all subsequent moves. Then, after each move, the budget is reduced by the number of nodes actually searched, and possibly topped up by the given time increment (`availableNodes -= searchedNodes - inc * nodestime`).
